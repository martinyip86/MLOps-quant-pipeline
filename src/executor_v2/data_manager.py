...
class DataManager:
    ...
    @staticmethod
    def _micro_price(bid_price:float,bid_volume:float,ask_price:float,ask_volume:float) -> float:
        return (bid_price * ask_volume + ask_price * bid_volume) / (bid_volume + ask_volume)
    
    @staticmethod
    def _mid_price(bid_price:float,ask_price:float) -> float:
        return (bid_price + ask_price) / 2
    
    @staticmethod
    def _obi(bid_volumes:list[float],ask_volumes:list[float],level:int=1) -> float:
    ...
    def _update_trades(self,symbol:str,mkt_type:str,data_type:str,data:dict):
        ...
        signed_amount = data["amount"] if side == "buy" else -turnover
        ...
        now_ms = data["timestamp"]
        while data_bucket and data_bucket["timestamp"] < (now_ms - 3000):
            data_bucket.popleft()

    def _update_market_price(self,symbol:str,data_type:str,mkt_type:str,data:dict):
        data_bucket = self.data_bucket[symbol][data_type][mkt_type]
        data_bucket.append({
            "timestamp":data["timestamp"],
        ...
        now_ms = data["timestamp"]
        while data_bucket and data["timestamp"] < (now_ms - 3000):
            data_bucket.popleft()

    def _update_open_interest(self,symbol:str,data_type:str,mkt_type:str,data:dict):
        data_bucket = self.data_bucket[symbol][data_type][mkt_type]
        data_bucket.append({
            "timestamp":data["timestamp"],
    ...
    def generate_features(self,symbol:str):
        ...
        orderbook_swap = data_bucket["orderbook"]["swap"]
        trades_spot = data_bucket["trades"]["spot"]
        trades_swap = data_bucket["trades"]["swap"]
        mark_price = data_bucket["market_price"]["swap"]
        open_interest = data_bucket["open_interest"]["swap"]

        for dt in [orderbook_spot,orderbook_swap,trades_spot,trades_swap,mark_price,open_interest]:
            if not self._is_window_ready(dt,2000): return None

        features = {
            "best_bid":orderbook_swap[-1]["bid_price"],
            "best_ask":orderbook_swap[-1]["ask_price"],
            "micro_price_swap":orderbook_swap[-1]["micro_price"],
            "mid_price_swap":orderbook_swap[-1]["mid_price"],
            "mid_price_spot":orderbook_spot[-1]["mid_price"],
            "spread_swap":orderbook_swap[-1]["spread"],
            "spot_obi_l1":orderbook_spot[-1]["obi_l1"],
            "spot_obi_l5":orderbook_spot[-1]["obi_l5"],
            "swap_obi_l1":orderbook_swap[-1]["obi_l1"],
            "swap_obi_l5":orderbook_swap[-1]["obi_l5"],
            ...
            "swap_ob_ofi_1s":self._ofi_window(orderbook_swap,window_ms=1000),
            "swap_ob_ofi_2s":self._ofi_window(orderbook_swap,window_ms=2000),
            "swap_trades_flow_1s":self._trades_flow_window(trades_swap,window_ms=1000),
            "swap_trades_flow_2s":self._trades_flow_window(trades_swap,window_ms=2000),
            "spot_trades_flow_1s":self._trades_flow_window(trades_spot,window_ms=1000),
            "spot_trades_flow_2s":self._trades_flow_window(trades_spot,window_ms=2000)
        }

        snapshot = {
            "spot_orderbook":orderbook_spot[-1],
            "swap_orderbook":orderbook_swap[-1]
        }
    ...
    @staticmethod
    def _ofi_window(orderbook:deque,window_ms:int) -> float:
        if not orderbook: return 0.0

        now_ms = orderbook[-1]["timestamp"]
        start_ms = now_ms - window_ms

        rows = [row for row in orderbook if row["timestamp"] >= start_ms]
        ...
        for prev,cur in zip(rows,rows[1:]):
            ...
            if cur["ask_prie"] < prev["ask_price"]:
    ...
    @staticmethod
    def _trades_flow_window(trades:deque,window_ms:int) -> float:
